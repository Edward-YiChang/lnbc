...
| Item | Files to run (in order) | Numeric source | Contents |
|---|---|---|---|
| Table 1 | `simulation/make_parameters.R` | `simulation/summary/table1_parameters.csv` | Simulation parameters and true accuracy measures. |
| Table 2 | `run_all.sh`, then `simulation/summarize_results.R` | `simulation/summary/point_summary.csv` | Point-estimation summaries for `pi = 0.90`. |
| Table 3 | `run_all.sh`, then `simulation/summarize_results.R` | `simulation/summary/coverage_summary.csv` | Coverage probabilities and average interval lengths for `pi = 0.90`. |
| Table 4 | `real data/malaria.R`, then `real data/summarize_scaled_results.R` | `real data/biomarker_div_100000/malaria_div100000_point_estimates.csv` | Malaria point estimates and percentile bootstrap intervals. |
| Table S1 | `run_all.sh`, then `simulation/summarize_results.R` | `simulation/summary/coverage_summary.csv` | Estimated-k versus fixed-k coverage; select `EMBC` and `EMBC_fixk`. |
| Table S2 | `gof/gof_symm.R`, then `gof/summarize_gof_symm.R` | `gof/result_symm/summary/gof_summary.csv` | Lognormal GOF rejection rates; use the `sup3_quantile` rows. |
| Table S3 | `run_all.sh`, then `simulation/summarize_results.R` | `simulation/summary/point_summary.csv` | Point-estimation summaries for `pi = 0.75`. |
| Table S4 | `run_all.sh`, then `simulation/summarize_results.R` | `simulation/summary/coverage_summary.csv` | Coverage probabilities and average interval lengths for `pi = 0.75`. |

The unified simulation is shared by Tables 2, 3, S1, S3, and S4; it only
needs to be run once. If the raw results already exist, use only the summary
script. For Table S2, summarize after completing the GOF replications.
