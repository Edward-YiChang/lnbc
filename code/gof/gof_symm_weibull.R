...
#   pi0 = pi1 = 0.90, 0.75;
#   J = 0.3, 0.5; and
#   homo/heterogeneous Weibull distributions with shapes (0.5, 0.5)
#   and (0.5, 0.25), respectively. Scales are fixed at the homogeneous
#   defaults, so J labels refer to the homogeneous baseline settings.

usage <- function() {
  cat(
    "Usage: Rscript gof/gof_symm_weibull.R --rep-id N [options]\n\n",
    ...
    "                   Default: 0.9,0.75.\n",
    "  --J X,Y,...     Baseline J values from 0.3,0.5. Default: 0.3,0.5.\n",
    "  --scenario S    both, homo, or hete. Default: both.\n",
    "  --repo-root DIR  Repository root. Default: parent of this script.\n",
    "  --out-dir DIR    Output root. Default: gof/result_symm_weibull.\n",
    "  --help           Show this help.\n",
    sep = ""
  )
}
...
settings <- list(
  ...
  out_dir = file.path(script_dir, "result_symm_weibull")
)
  ...
  for (pi in pi_grid) {
    for (J in J_grid) {
      for (scenario in scenarios) {
        ...
        parameters <- list(
          a0 = 0.5,
          a1 = if (J == 0.3) 2.6793 else 9.7345,
          b0 = 0.5,
          b1 = if (scenario == "homo") 0.5 else 0.25
        )
        ...
        result$b0 <- parameters$b0
        result$b1 <- parameters$b1
        result$sdlog0 <- NA_real_
        result$sdlog1 <- NA_real_
